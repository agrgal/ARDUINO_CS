...
void setup()
{
  ...
  DDRD = B10000000;  // digital pins 7,6,5,4,3,2,1,0  ¿El pin 7 es de Salida?
...
void loop()
{
  ...
  if (juego) { // Comprueba el estado del juego
    ...
    if (respuestas!=cables) { 
      ...
      tocarContinuaFlag=1; // activo la bandera  
    }
      ...
      escribeRespuestas(respuestas); // Se escriben las respuestas
          ...
          if (solucionElegidaByte==respuestas) { // Detectar ganador
            tocaMusica(3,100); // musica de ganador
            parpadeo(respuestasTemp, cables); // parpadeo elegido
            lcd.clear();
            lcd.setCursor(0,0);
            ...
            tocaMusica(4,100); // musica de perdedor
            parpadeo(respuestasTemp, cables);
          ...
          } else if (tocarContinuaFlag==1){
            // musica de continua
            tocarContinuaFlag=0; // Desactivo bandera
            tocaMusica(2,100); // musica de continuar
            parpadeo(respuestasTemp, cables);
          }    
