...
unsigned int sample;
const int minimoAdmisible = 50; //valor mínimo admisible de decibelios. El nivel del "Silencio". Obtener este valor experimentalmente.
...
void encender(long dato) {
  ...
  ajuste = map(ajuste,0,1023,1,300-minimoAdmisible); // Valores de decibelios considerados como máximo, en diferencia. 1, para que no divida entre cero
  int valorDeCalculo = (dato - minimoAdmisible)*((dato - minimoAdmisible)>0); // diferencia con unos valores mínimos...

  int cual = map(valorDeCalculo,0,ajuste,0,10); // Elijo cual es el que me corresponde... Pero a lo mejor no hace falta
  cual = cual*(cual<10)+10*(cual>=10); // lo corta en 10.
  float x = 2.0-((valorDeCalculo>=0 and valorDeCalculo<=ajuste)*(2.0*valorDeCalculo/ajuste)+(valorDeCalculo>ajuste)*2.0); // Cuando dato = 0, entonces x=2 y x va bajando...
  ...
  Serial.println("Ajuste: "+ (String) ajuste + "  DB(sobre admisible): "+(String) valorDeCalculo +" --> "+ (String) x +" : RGB("+ (String) red +"," +(String) green +","+ (String) blue + ") cualitativo: " + (String) cual ) ;
}
