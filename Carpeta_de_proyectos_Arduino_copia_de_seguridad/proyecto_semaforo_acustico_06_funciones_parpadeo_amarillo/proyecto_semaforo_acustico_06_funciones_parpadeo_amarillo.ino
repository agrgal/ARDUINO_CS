...
float colorIntermedio[]={0.0,50.0,50.0}; // colores que se alcanzan en el valor intermedio (1) RGB
int numPin = 2;
...
void encender(long dato) {
  ...
  if (x>=0.95 && x<=1.05) {
    analogWrite(pinElegido[0],0); // Escribo en el pin primero el valor de los rojos. Valor absoluto corta los valores negativos
    analogWrite(pinElegido[1],(int) ((millis()/200)%2)*colorIntermedio[1]); // Escribo en el pin primero el valor de los rojos
    analogWrite(pinElegido[1],(int) ((1+(millis()/200))%2)*colorIntermedio[2]); // Escribo en el pin primero el valor de los rojos
  } else {
