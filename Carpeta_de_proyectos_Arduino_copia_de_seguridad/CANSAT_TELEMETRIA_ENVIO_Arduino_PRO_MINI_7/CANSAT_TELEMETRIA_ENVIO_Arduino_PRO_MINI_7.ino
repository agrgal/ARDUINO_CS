...
Adafruit_BMP085 bmp;  
SoftwareSerial gps(3, 4); // Iniciamos el GPS (rxPIN --> TX Amarillo GPS, txPIN  --> RX Azul GPS)..
...
void loop() {
  ...
  */
  ...
  if (int(t/2350)>int(tAnt/2350)) { enviarDatos(cadenaParaEnviar( 1+(t/2350)%2)); }
  
  tAnt = t;
  
  digitalWrite(led, ((t/2500)%2)*((t/200)%2)*(altura<= 100));                                             //Hace parpadear al LED 2 veces cada 10 segundos.
  // digitalWrite(6, LOW); 
...
void extraerCadenaGPS(){
  if(gps.available()){
    String cadenaFinal = gps.readStringUntil('\n');                                                         //Leemos lo que nos ha llegado hasta el salto de línea. 
    if (cadenaFinal.startsWith("$GPRMC")) {
      int pos = cadenaFinal.indexOf("$GPRMC");                                                              //Si empieza por el código que queremos, pillamos la posición de inicio. 
      if (pos == 0) {
        cadenaGPS = cadenaFinal;
        Serial.println(cadenaFinal);                                                                        //Si la posición es 1, significa que tenemos toda la cadena, así que la recortamos y la imprimimos. 
      }
    }
  }
}
