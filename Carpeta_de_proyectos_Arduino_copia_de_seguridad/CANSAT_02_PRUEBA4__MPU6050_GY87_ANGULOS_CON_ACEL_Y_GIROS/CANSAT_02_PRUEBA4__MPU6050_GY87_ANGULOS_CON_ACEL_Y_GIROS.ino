...
int16_t ax, ay, az;
int16_t gx, gy, gz;
float angX, angZ, angY; // ángulos según acelerómetro
float angX_G, angY_G,angZ_G;
float angX_G0=0.0, angY_G0 =0.0 , angZ_G0=0.0; // ángulos según giroscopio
...
void loop() {
    ...
    angX = atan(ax/sqrt(ay*ay+az*az))*(180/pi);
    angY = atan(ay/sqrt(ax*ax+az*az))*(180/pi);
    ...
    angY_G0 = angY_G; // paso siguiente
    angZ_G0 = angZ_G; // paso siguiente

    // Mirar la corrección con el drift.
    // *********************************

    
    contador = int(millis()/1000);
    if (contador!=contadorAnt) {
        ...
        Serial.println("Ángulo Y: " + String (angY) + "º // Ángulo Z: " + String(angZ)+"º");    
        Serial.println("Ángulo Y: " + String (angY_G) + "º // Ángulo Z: " + String(angZ_G) +"º");
        Serial.println(); 
